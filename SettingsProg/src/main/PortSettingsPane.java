...
public class PortSettingsPane  extends GridPane {
	...
	public void setPort(String pname) {
		...
		for(int i = 0; i <cboPort.getItems().size(); i++) {
			System.out.println(cboPort.getItems().get(i).display + ", " + cboPort.getItems().get(i).value);
			if(cboPort.getItems().get(i).value.equals(pname)) {
				
				cboPort.setValue(cboPort.getItems().get(i));
	...
	public void setBaud(Integer b) {
		if (cboBaud.getItems().contains(b)) {
			cboBaud.setValue(b);
		}
	}
	...
	public void setParity(Integer p) {
		for(int i = 0; i<cboParity.getItems().size(); i++) {
			if(cboParity.getItems().get(i).value == p) {
				cboParity.setValue(cboParity.getItems().get(i));
	...
	public void setDataBits(Integer db) {
		if(cboDataBits.getItems().contains(db)) {
			cboDataBits.setValue(db);
		}
	}
	...
	public void setStopBits(Integer sb) {
		for(int i = 0; i<cboStopBits.getItems().size(); i++) {
			if(cboStopBits.getItems().get(i).value == sb) {
				cboStopBits.setValue(cboStopBits.getItems().get(i));
	...
	public void setFlowControl(Integer fl) {
		for(int i = 0; i< cboFlowControl.getItems().size(); i++) {
			if(cboFlowControl.getItems().get(i).value == fl) {
				cboFlowControl.setValue(cboFlowControl.getItems().get(i));
